...
public class MegaBookBuilder : MonoBehaviour
{
    ...
    public Texture2D GetPageTexture(int p, bool front)
    {
        ...
        if (front)
            return (Texture2D)mats[0].GetTexture("_MainTex");
        else
            return (Texture2D)mats[1].GetTexture("_MainTex");
    }
    ...
    public void SetPageTexture(Texture2D tex, int p, bool front)
    {
        ...
        if (front)
            ...
            mats[1].SetTexture("_MainTex", tex);
    }
	...
	{
		if(!MenuController.isPaused && !TextConverter.isOpen){
		audioSource.Play();
	...
	public void GoToPage(float npage){
