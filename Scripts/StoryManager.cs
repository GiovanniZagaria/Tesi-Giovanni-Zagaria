...
public class StoryManager : MonoBehaviour
{
    ...
    // Aggiunge una scelta alla storia e invia solo la scelta all'IA
    public void AddChoice(string choice)
    {
        if (!string.IsNullOrEmpty(choice))
        {
            ...
            // Invia solo la scelta all'IA, non tutta la storia
            if (flaskManager != null)
            {
                flaskManager.SendChoiceAndStoryToAI(choice, "", OnAISuccess); // Invia solo la scelta, la storia rimane separata
                Debug.Log("Choice sent to AI: " + choice);
            }
            else
            {
                Debug.LogError("FlaskManager è null, impossibile inviare la scelta all'IA.");
            }
        }
    }
